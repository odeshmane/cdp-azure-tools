...
az network nsg rule create -g $RESOURCEGROUPNAME --nsg-name "$ENVIRONMENTNAME-NSG" -n "cdp-mgmtplane" --priority 300 \
   --source-port-ranges 9443 --destination-address-prefixes '*' --destination-port-ranges 9443 --access Allow --protocol Tcp --description "Allow CDP Management Plane to access over Port 8443"
